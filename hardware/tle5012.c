...
uint32_t address= SAMPLING_DATA_ADDR ;//校准数据写入FLASH的指定地址
...
void tle5012b_init(void)
{
    ...
    GPIO_InitStructure.GPIO_Pin =GPIO_Pin_15;				 //输入模式
...
uint16_t ReadValue(uint16_t RegAdd)
{
    ...
    SPI_I2S_SendData(SPI1, RegAdd); //通过外设SPIx发送一个数据
...
{   
  ...
  OneStep();
  delay_ms(1000); 
  for(x=199;x>=0;x--)//顺时针跑一圈记下各整步位的值和之前逆时针保存的值求平均
  {    
    encoderReading=0;
   	delay_ms(20);                     
    lastencoderReading=ReadAngle();     
  ...
  if(Second_Calibrate_flag !=1){                            //如果是二次校准就不存储flash
    ...
    Calibration_flag =0xAACC;    //标记已校准标志
