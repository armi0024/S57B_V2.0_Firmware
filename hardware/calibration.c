...
static uint16_t interp(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t x)
{
	int32_t dx,dy,dx2,y;
	dx=x2-x1;
	dy=y2-y1;
...
uint8_t updateTableValue(int32_t index, int32_t value)
{
...
void printCalTable(void)
{
	...
	{
		printf("table[i].value = %d\r\n",(uint16_t)table[i].value);
		printf(",");
	}
	printf("\n\r");
}

uint16_t fastReverseLookup(uint16_t encoderAngle)
{
#ifdef NZS_FAST_CAL
	...
	if (fastCalVaild)
	{
		...
		return (uint16_t)NVM->FastCal.angle[x];
	}else
	{
...
uint16_t reverseLookup(uint16_t encoderAngle)
{
	...
	printf("WE did some thing wrong\r\n");
...
//	data.status=1;
	...
	printf("Writting Calbiration to Flash");
...
//	FlashCalData_t data;
//	int i;
	printf("Reading Calbiration to Flash");
//	memcpy(&data, &NVM->CalibrationTable,sizeof(data));
//	for (i=0; i<CALIBRATION_TABLE_SIZE; i++ )
//	{
...
uint8_t flashGood(void)
{
//	printf("calibration table status is: %d",NVM->CalibrationTable.status);
//    printf("calibration table status is: %d",status);
	return 0;//NVM->CalibrationTable.status;
}
...
void createFastCal(void)
{
	...
	j=0;
	cs=0;
	printf("setting fast calibration");//创建快速刻度表
	for (i=0; i<16384; i++)
	{
...
//			flashWrite(&NVM->FastCal.angle[i-255],data,256*sizeof(uint16_t));
			printf("Wrote fastcal at index %d-%d", i-255, i);
			j=0;
		}
		cs+=x;
	}
...
void updateFastCal(void)
{
	...
	if (cs!=NVM->FastCal.checkSum || NonZero==0)
	{
		createFastCal();        //创建刻度表
	}
	else
	{
		printf("fast cal is valid");
		fastCalVaild=1;
	}
#endif
}
...
void updateTable(uint16_t actualAngle, uint16_t encoderValue)
{
	int32_t dist, index;
	uint16_t tableAngle;
...
int getValue(uint16_t actualAngle, CalData_t *ptrData)
{
...
uint16_t getCal(uint16_t actualAngle)
{
	CalData_t data;
