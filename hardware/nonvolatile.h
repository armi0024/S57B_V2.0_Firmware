 ...
 *********************************************************************/
#ifndef __NONVOLATILE__H
#define __NONVOLATILE__H
...
typedef struct {
	...
	uint8_t motorWiring;  //forward wiring of motor or reverse
...
typedef struct {
	...
	uint8_t errorLogic; //if high and error will be high on output pin
...
uint8_t nvmWriteCalTable(void *ptrData, uint32_t size);
uint8_t nvmWrite_sPID(float Kp, float Ki, float Kd);
uint8_t nvmWrite_pPID(float Kp, float Ki, float Kd);
uint8_t nvmWrite_vPID(float Kp, float Ki, float Kd);
uint8_t nvmWriteSystemParms(SystemParams_t systemParams);
uint8_t nvmWriteMotorParms(MotorParams_t motorParams);
uint8_t nvmErase(void);
