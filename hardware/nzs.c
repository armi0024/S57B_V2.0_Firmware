...
{
	printf("called motorCurrent %d",argc);
	if (argc==1)
	{
		int i;
		printf("called %s",argv[0]);
		i=atol(argv[0]);
		i=i*100;
		MotorParams_t params;
		memcpy((void *)&params, (void *)&NVM->motorParams, sizeof(params));
	...
	printf(" motorCurrent return %d",i);
	return i;
...
static void enableInput(void)
{
	...
	if (NVM->SystemParams.errorPinMode == ERROR_PIN_MODE_ENABLE)
	{
		static int enable;
		//read our enable pin
		enable = 1;// digitalRead(PIN_ENABLE);
		if (enable != enableState)
		{
			printf("Enable now %d",enable);
		}
		enableState=enable;
	...
	if (NVM->SystemParams.errorPinMode == ERROR_PIN_MODE_ACTIVE_LOW_ENABLE)
	{
		static int enable;
		//read our enable pin
		enable = !0;//digitalRead(PIN_ENABLE);
		if (enable != enableState)
		{
			printf("Enable now %d",enable);
		}
...
//	{
		...
		error=processFeedback(); //handle the control loop
		YELLOW_LED(error);
#ifdef PIN_ENABLE
...
//	commandsInit(); //setup command handler system
	...
	{
		printf("init the stepper controller");
		stepCtrlError=begin(); //start controller before accepting step inputs

		//todo we need to handle error on LCD and through command line
		if (STEPCTRL_NO_POWER == stepCtrlError)
		{

			printf("Appears that there is no Motor Power\r\n");
			printf("Connect motor power!\r\n");
#ifndef DISABLE_LCD
...
//			Lcd.setMenu(NULL);
		...
		{
			printf("AS5047D not working");
			printf(" try disconnecting power from board for 15+mins");
			printf(" you might have to short out power pins to ground");
#ifndef DISABLE_LCD
...
void loop(void)
{
	...
	if (enableState != getEnable())
	{
		enable(enableState);
	}

	//handle EEPROM
	eepromData.angle=getCurrentAngle();
	eepromData.encoderAngle=getEncoderAngle();
	eepromData.valid=1;
