 ...
 *********************************************************************/
#ifndef __CALIBRAITON_H
#define __CALIBRAITON_H
...
typedef struct {
	uint16_t table1[CALIBRATION_TABLE_SIZE];
	uint8_t status;
} FlashCalData_t;
...
//  private:
    ...
    uint8_t flashGood(void); //returns true if the flash copy of calibration is valid
...
//  public:
    ...
    uint16_t reverseLookup(uint16_t encoderAngle); //this turns encoder angle into real angle
