...
volatile uint8_t TC5_ISR_Enabled=0;
...
volatile uint8_t enableFeedback; //true if we are using PID control algorithm

volatile int32_t locReadIndx=0;
volatile int32_t locWriteIndx=0;
volatile uint8_t  enabled1;
...
uint16_t getStartupEncoder(void)
{
    return startUpEncoder;
}
int32_t getLoopError(void)
{
    return loopError;
}
...
feedbackCtrl_t getControlMode(void) 
{ 
    return systemParams.controllerMode;
}
uint8_t calibrationValid(void)
{ 
    return calValid();
}  //returns true if calbiration is good
...
static uint8_t enterCriticalSection()
{
	uint8_t state=TC5_ISR_Enabled;
//	disableTCInterrupts();
	return state;
}

static void exitCriticalSection(uint8_t prevState)
{
	if (prevState)
	{
//		enableTCInterrupts();
...
void updateParamsFromNVM(void)                  //更新参数来自NVM 
{
...
void setLocationFromEncoder(void)
{
	...
	if (calValid())
	{
		...
		printf("start angle after rounding %d %d.%03d\r\n", (uint16_t)a,x/1000,x%1000);
...
{
	int32_t angle1=0,angle2=0,x=0,i;
	uint8_t feedback=enableFeedback;
	int32_t microsteps=systemParams.microsteps;

	systemParams.microsteps=1;
	enableFeedback=0;
	motorParams.motorWiring=1; //assume we are forward wiring to start with
	setRotationDirection(motorParams.motorWiring);
	/////////////////////////////////////////
	...
	printf("sample encoder\r\n");

	angle1=sampleMeanEncoder(200);

	printf("move");   //A4954_NUM_MICROSTEPS =256//
	move(256/2,motorParams.currentMa); //move one half step 'forward'
	delay_ms(100);
	move(256,motorParams.currentMa); //move one half step 'forward'
	delay_ms(500);
	printf("sample encoder");
	angle2=sampleMeanEncoder(200);

	printf("Angles %d %d",angle1,angle2);
	...
	printf("%%angle delta %d %d (%d %d)",x,abs(angle2-angle1),angle1,angle2 );

	//move back
	move(-A4954_NUM_MICROSTEPS/2,motorParams.currentMa); //move one half step 'forward'
	delay_ms(100);
	move(-A4954_NUM_MICROSTEPS,motorParams.currentMa); //move one half step 'forward'
...
{
//	printf("current %d desired %d %d",(int32_t) currentLocation, (int32_t)getDesiredLocation(), numSteps);
 ...
 */
uint16_t maxCalibrationError(void)
{
	//Angle startingAngle;
	uint8_t done=0;
	int32_t mean;
	...
	enableFeedback=0;
	j=0;
	printf("Running calibration test");
	...
	while(!done)
	{
		...
		cal=getCal(desiredAngle);
		dist=(uint16_t)(mean)-cal;
		act=fastReverseLookup(cal);

		printf("actual %d, cal %dr\rn",mean,(uint16_t)cal);
		printf("desired %d\r\n",(uint16_t)desiredAngle);
		printf("numSteps %lld\r\n", numSteps);

		printf("cal error for step %d is %d\r\n",j,dist);
		printf("mean %d, cal %d\r\n",mean, (uint16_t)cal);
		...
		move(steps,motorParams.currentMa);
	...
	enableFeedback=feedback;
	if (state) enableTCInterrupts();
	printf("max error is %d cnts\r\n", maxError);
	return ((uint16_t)maxError);
}
...
uint8_t calibrateEncoder(void)
{
	...
	{
		int ii,N;
		uint16_t cal,desiredAngle;
		desiredAngle=(uint16_t)(getDesiredLocation() & 0x0FFFFLL);
		cal=getCal(desiredAngle);
		delay_us(200);
		mean=sampleMeanEncoder(200);

		printf("Previous cal distance %d, %d, mean %d, cal %d",j, cal-((uint16_t)mean), mean, (uint16_t)cal);

		updateTableValue(j,mean);
	...
	//calTable.printCalTable();
	saveToFlash(); //saves the calibration to flash
	printCalTable();
...
stepCtrlError_t begin(void)
{
	...
	printf("start up encoder\r\n");
//	if (0 == encoder.begin(PIN_AS5047D_CS))
//	{
//		return STEPCTRL_NO_ENCODER;
//	}

	printf("cal table init\r\n");
	init();

	startUpEncoder=(uint16_t)getEncoderAngle();
	printf("start up encoder %d\r\n",startUpEncoder);

	printf("start stepper driver\r\n");
//	stepperDriver.begin();
...
#else
	printf("measuring step size\r\n");
	x=measureStepSize();
	if (abs(x)<0.5)
	{
		printf("Motor may not have power\r\n");
		return STEPCTRL_NO_POWER;
	}
#endif


	printf("Checking the motor parameters\r\n");
	//todo we might want to move this up a level to the NZS
	//  especially since it has default values
	if (0 == NVM->motorParams.parametersVaild)
	{
		MotorParams_t params;
		printf("NVM motor parameters are not set, we will update");
	...
	printf("Motor params are now good\r\n");
	printf("fullSteps %d\r\n", motorParams.fullStepsPerRotation);
	printf("motorWiring %d\r\n", motorParams.motorWiring);
	printf("currentMa %d\r\n", motorParams.currentMa);
	printf("holdCurrentMa %d\r\n", motorParams.currentHoldMa);
	printf("homeMa %d\r\n", motorParams.homeMa);
	printf("homeHoldMa %d\r\n", motorParams.homeHoldMa);
...
uint16_t sampleAngle(void)
{
	...
	y=readEncoderAnglePipeLineRead(); //convert the 14 bit encoder value to a 16 bit number
	x=readEncoderAnglePipeLineRead();
...
uint16_t sampleMeanEncoder(int32_t numSamples)
{
	...
	for (i=0; i<(numSamples); i++)
	{
		...
		//printf("i %d,min %d, max %d, last %d, x %d", i, min, max, last, x);
	...
	printf("min %d, max %d, mean %d\r\n", min, max, (int32_t)(mean/numSamples));
	return (uint16_t)(mean/numSamples);
}

void feedback(uint8_t enable)
{
	disableTCInterrupts();
	motorReset();
...
void requestStep(int dir, uint16_t steps)
{
	uint8_t state;
...
void move1(int dir, uint16_t steps)
{
	int64_t ret;
	int32_t n;
...
void moveToAngle(int32_t a, uint32_t ma)
{
	...
	//LOG("move %d %d",a,ma);
//	stepperDriver.move(a,ma);//调用电流设置的额move
...
uint16_t getEncoderAngle(void)
{
	uint16_t a;
	uint8_t state=enterCriticalSection();
	a=fastReverseLookup(sampleAngle());
	exitCriticalSection(state);
	return a;
}
...
uint8_t vpidFeedback(int64_t desiredLoc, int64_t currentLoc, Control_t *ptrCtrl)
{
	int32_t fullStep=ANGLE_STEPS/motorParams.fullStepsPerRotation;
	int64_t lastY = getCurrentLocation();
...
uint8_t pidFeedback(int64_t desiredLoc, int64_t currentLoc, Control_t *ptrCtrl)
{
	static int count=0;
...
uint8_t determineError(int64_t currentLoc,int64_t error)
{
...
uint8_t simpleFeedback(int64_t desiredLoc, int64_t currentLoc, Control_t *ptrCtrl)
{
	...
	static uint8_t lastProbeState=0;
	...
	if (enableFeedback)
	{
		...
		if (systemParams.homePin>=0)
		{
			...
			if (0)//digitalRead(systemParams.homePin)==
			{
				if (lastProbeState==0)
				{
	...
	//errorCount=0;
//	limitCurrent(99); //reduce noise on low error
	return 0;
...
//	enable(enable); //enable or disable the stepper driver as needed


	if (enabled1==1 && enable==0)
	{
		feedback = 0;
	}
	if (enabled1==0 && enable==1) //if we are enabling previous disabled motor
	{
		feedback = 1;
...
//	us=micros();
	...
	int64_t x=16;           //
//	x=getSteps()-lastSteps;//前后两次接收到的步数作差
	updateSteps(x);
	lastSteps+=x;
#endif
...
void PID_Autotune(void)
{
	...
	printf("Thres %d, start %d \r\n",(int32_t)thres,(int32_t)startAngle);
	...
	for (i=0; i<100; i++)
	{
		...
		move(A4954_NUM_MICROSTEPS,force);
