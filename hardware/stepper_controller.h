...
//	public:
		...
		int32_t getLoopError(void); //assume atomic read

		uint8_t calibrationValid(void);  //returns true if calbiration is good

		void updateParamsFromNVM(void);  //updates the parameters from NVM
//		CalibrationTable calTable;
		//void printData(void);

		uint8_t calibrateEncoder(void); //do manual calibration of the encoder
		uint16_t maxCalibrationError(void); //measures the maximum calibration error as an angle
		...
		uint8_t processFeedback(void); // does the feedback loop
		...
		void move1(int dir, uint16_t steps); //forces motor to move even if feedback controller is turned off.
