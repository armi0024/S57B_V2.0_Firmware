...
//则波特率为:36M/((1+6+5)*6)=500Kbps 波特率一定要设置对，否则通信一定失败
//返回值:0,初始化OK;
//    其他,初始化失败; 
...
uint8_t CAN1_Mode_Init(uint8_t tsjw,uint8_t tbs2,uint8_t tbs1,uint16_t brp,uint8_t mode)
{
    ...
    GPIO_Init(GPIOA, &GPIO_InitStructure);//初始化PA11,PA12
	  ...
	  //引脚复用映射配置
//	  GPIO_PinRemapConfig( GPIO_Remap1_CAN1,  ENABLE); //GPIOA11复用为CAN1
    ...
    NVIC_EnableIRQ(USB_LP_CAN1_RX0_IRQn);//接收完数据将中断重新开启
...
{   static int i=0;
    ...
    CAN_ClearITPendingBit(CAN1,CAN_IT_FMP0);//清除FIFO消息挂号中断标志位
	
    CAN_Receive(CAN1, 0, &RxMessage);
    //可以添加发送的数据ID格式是否为标准帧格式
    
	for(i=0;i<8;i++)
        printf("rxbuf[%d]:%#x\r\n",i,RxMessage.Data[i]);
    
    CAN_FIFORelease(CAN1,CAN_FIFO0);//释放FIFO0数据缓存区
    NVIC_EnableIRQ(USB_LP_CAN1_RX0_IRQn);//接收完数据将中断重新开启
}
#endif
